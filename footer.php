...
// @beat — Swiss Internet Time z času poslednej aktualizácie (BMT = UTC+1, 1 deň = 1000 beatov)
if (defined('DEPLOY_TIMESTAMP')) {
    $updateTs = (int) DEPLOY_TIMESTAMP;
} else {
    $currentFile = $_SERVER['SCRIPT_FILENAME'] ?? '';
    $updateTs = ($currentFile !== '' && file_exists($currentFile)) ? (int) filemtime($currentFile) : time();
}
$bmtSecs    = ($updateTs + 3600) % 86400;
$beatValue  = $bmtSecs / 86.4;
$swatchBeat = '@' . number_format($beatValue, 2, '.', '');
?>
    ...
    <div class="container site-footer__body">
      ...
      <nav class="site-footer__col site-footer__col--nav" aria-label="Rýchla navigácia v pätičke">
        ...
        <ul class="site-footer__nav-list">
          ...
          <li><a href="nastroje.php" class="site-footer__link">Nástroje</a></li>
