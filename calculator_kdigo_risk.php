        ...
        if (!isLoggedIn()) {
            ...
            if ($resultId <= 0) {
                ...
                try {
                    ...
                    error_log(
                        "calculator_kdigo_risk delete error: " .
                            $e->getMessage(),
                    );
                }
            }
...
<body>
                        ...
                        <table class="admin-table">
                            ...
                            <tbody>
                                <?php foreach ($savedResults as $row): ?>
                                    <?php
                                    $input = is_array($row["input_payload"])
                                        ? $row["input_payload"]
                                        : [];
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <?php if (
                                                !empty($result["g_category"]) &&
                                                !empty($result["a_category"])
                                            ): ?>
                                                (<?= htmlspecialchars(
                                                    (string) $result[
                                                        "g_category"
    ...
    (string) $result["a_category"],
) ?>)
                                            <?php endif; ?>
                                            <?php if (
                                                !empty($input["egfr"]) &&
                                                !empty($input["uacr_mg_g"])
                                            ): ?>
                                                - eGFR <?= htmlspecialchars(
                                                    (string) $input["egfr"],
                                                ) ?>,
                                                UACR <?= htmlspecialchars(
                                                    number_format(
                                                        (float) $input[
                                                            "uacr_value"
                                                        ],
