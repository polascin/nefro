/*
 * nastroj_aki.js — Interaktívny diferenciálne-diagnostický sprievodca AKI.
 *
 * Súčasťou je generický engine rozhodovacieho stromu (NefroDecisionTree),
 * ktorý sa dá znovupoužiť pre ďalšie nástroje (hyponatrémia, hypokaliémia …) —
 * stačí mu odovzdať mount element a dátový strom rovnakého tvaru.
 *
 * Bez inline štýlov (CSP style-src 'self') — všetko cez triedy v index.css.
 * Bez závislostí, vanilla JS, defer.
 */
(function () {
    'use strict';
     ...
     * @param {Object} tree        { start: 'id', nodes: { id: node, ... } }
     */
    function NefroDecisionTree(mount, tree) {
        ...
        function renderOutcome(node, container) {
            ...
            container.appendChild(el('p', 'tool-outcome__note',
                'Záver je orientačný a vychádza zo zadaných odpovedí. Vždy posúďte celkový klinický kontext, '
                + 'močový sediment a laboratórium; mnohé AKI majú zmiešanú etiológiu.'));
            return h;
        }
        ...
        function render() {
            ...
            // Presun fokus na nadpis kroku (ak používateľ klikol — nie pri prvom načítaní)
    ...
    // ── Dátový strom: diferenciálna diagnostika AKI ────────────────────────
    var AKI_TREE = {
        start: 'confirm',
        nodes: {
            confirm: {
    ...
    function init() {
        var mount = document.getElementById('aki-tool');
        if (mount) { NefroDecisionTree(mount, AKI_TREE); }
    }
    ...
    // Sprístupnenie enginu pre prípadné ďalšie nástroje na stránke
