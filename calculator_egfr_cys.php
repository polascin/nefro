...
function egfrCysCategory(float $egfr): string
{
    if ($egfr >= 90.0) {
        return "G1";
    }
...
function egfrCysCategoryDescription(string $category): string
{
    $map = [
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!validateCsrfToken((string) ($_POST["csrf_token"] ?? ""))) {
        ...
        calculatorHandleDeleteSaved($pdo, $errors, $messages, 'calculator_egfr_cys');
        ...
        if (empty($errors)) {
            ...
            $scys = (float) $cystatinValue;

            // CKD-EPI 2021 kreatinín–cystatín C (Inker et al., NEJM 2021), bez rasy
            $kappa = $sex === "female" ? 0.7 : 0.9;
            $alpha = $sex === "female" ? -0.219 : -0.144;

            $crRatio = $creatinineMgDl / $kappa;
            $cysRatio = $scys / 0.8;

            $egfr =
                135 *
                pow(min($crRatio, 1), $alpha) *
                pow(max($crRatio, 1), -0.544) *
                pow(min($cysRatio, 1), -0.323) *
                pow(max($cysRatio, 1), -0.778) *
                pow(0.9961, (int) $ageYears);
            ...
            $gCategory = egfrCysCategory($egfrRounded);
            $gDescription = egfrCysCategoryDescription($gCategory);
                    ...
                    try {
                        ...
                        if (
                            calculatorSaveResult(
                                ...
                                "egfr_ckd_epi_cr_cys_2021",
                                "eGFR (CKD-EPI kreatinín–cystatín C 2021)",
                                $patient,
                                $inputPayload,
                                $resultPayload,
                            )
                        ...
                        error_log(
                            "calculator_egfr_cys save error: " . $e->getMessage(),
                        );
                    }
                }
            }
...
if (isLoggedIn()) {
    try {
        ...
        error_log("calculator_egfr_cys fetch history error: " . $e->getMessage());
    }
}
?>
<!DOCTYPE html>
...
<head>
  <?php
  $pageTitle = "eGFR CKD-EPI kreatinín–cystatín C 2021 | Kalkulačky | Nefro-projekt Slovensko";
      ...
      [
          ...
          "itemListElement" => [
              ...
              [
                  ...
                  "item" => $baseUrl . "calculator_egfr_cys.php",
              ],
          ],
      ],
  ];
  ...
  ?>
  <meta name="calculator-key" content="egfr_ckd_epi_cr_cys_2021">
</head>
<body>
    <a href="#main-content" class="skip-link">Preskočiť na hlavný obsah</a>

    <?php
    $headerTitle = "Kalkulačka eGFR (kreatinín–cystatín C)";
    ...
    <main id="main-content" class="container main-content main-content--single-col" role="main">
        <div class="content-wrapper">
            <div class="auth-container auth-container--wide">
                <h2>eGFR (CKD-EPI kreatinín–cystatín C 2021)</h2>
                <p class="auth-subtitle">Voliteľné údaje pacienta + povinné vstupy pre výpočet.</p>

                <div class="info-box">
                    <strong>Kedy použiť kombinovaný odhad:</strong> KDIGO 2024 odporúča
                    eGFR<sub>cr-cys</sub> ako presnejší odhad a na <em>potvrdenie</em> CKD,
                    keď má eGFR z kreatinínu klinické dôsledky (napr. na hranici rozhodnutia,
                    pri atypickej svalovej hmote, pred dávkovaním liekov). Kombinovaný vzorec
                    je spravidla presnejší než kreatinín alebo cystatín C samostatne.
                </div>

                <details open class="calc-formula-box">
                    <summary>Vzorec — CKD-EPI 2021 (kreatinín–cystatín C)</summary>
                    <div class="calc-formula-content">
                        <div class="calc-formula-line">\[ \begin{aligned} \text{eGFR} = 135 &\times \min(S_{cr}/\kappa, 1)^{\alpha} \times \max(S_{cr}/\kappa, 1)^{-0.544} \\ &\times \min(S_{cys}/0.8, 1)^{-0.323} \times \max(S_{cys}/0.8, 1)^{-0.778} \\ &\times 0.9961^{\text{Vek}} \times [0.963 \text{ ak žena}] \end{aligned} \]</div>
                        <div class="calc-formula-line">\[ S_{cr} [\text{mg/dL}] = S_{cr} [\mu\text{mol/L}] \div 88.4 \]</div>
                        <div class="calc-formula-vars">
                            $\kappa = 0.7 \text{ (žena)} / 0.9 \text{ (muž)}$ &bull;
                            $\alpha = -0.219 \text{ (žena)} / -0.144 \text{ (muž)}$ &bull;
                            $S_{cr} = \text{kreatinín [mg/dL]}$ &bull;
                            $S_{cys} = \text{cystatín C [mg/L]}$
                        </div>
                    </div>
                </details>
                ...
                <div class="info-box-green">
                    ...
                    <a href="https://nephroresource.com/calculators/ckdepi-cr-cys" target="_blank" rel="noopener noreferrer">NephroResource</a>
                </div>
                ...
                <form method="POST" action="calculator_egfr_cys.php">
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-grid">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" id="cystatin_value" name="cystatin_value" required class="form-control" value="<?= htmlspecialchars(
                                    $form["cystatin_value"],
                ...
                ?>
                    <div class="form-section calculator-result-block <?= htmlspecialchars($riskCls) ?>" role="status" aria-live="polite">
                        ...
                        <p class="calc-result-detail"><strong>Kreatinín (mg/dL):</strong>
                            ...
                            &ensp;&bull;&ensp;<strong>Cystatín C (mg/L):</strong>
                            <?= htmlspecialchars(number_format((float)$calculated["cystatin_value"], 2, ",", " ")) ?>
                        </p>
                        <div class="form-actions no-print">
                            <button type="button" class="btn-primary js-print">Vytlačiť výpočet</button>
                            <a href="calculator_history.php?calc=egfr_ckd_epi_cr_cys_2021" class="btn-secondary">História eGFR (kr-cys)</a>
                        </div>
                    </div>
                <?php endif; ?>
            </div>
