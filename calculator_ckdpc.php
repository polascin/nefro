...
function ckdpcInterpretation(float $risk3yr): array
{
    ...
    if ($risk3yr < 5.0) {
        ...
        $warnings[] = 'Riziko ≥10 % — zvážiť intenzívnejší/multidisciplinárny model starostlivosti (analogicky s prahom pre 2-ročné riziko z KFRE).';
    } else {
        $interpretation[] = '3-ročné riziko '
            . number_format($risk3yr, 1, ',', ' ')
            . ' % — vysoké riziko rýchlej progresie, prioritne zvážiť konzultáciu nefrológa a optimalizáciu terapie.';
        $warnings[] = 'Riziko ≥20 % — zvážiť včasné plánovanie náhrady funkcie obličiek a odoslanie na nefrologickú ambulanciu.';
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!validateCsrfToken((string) ($_POST['csrf_token'] ?? ''))) {
        ...
        if (!isLoggedIn()) {
            ...
            if ($resultId <= 0) {
                ...
                try {
                    ...
                    error_log('calculator_ckdpc delete error: ' . $e->getMessage());
                }
            }
        }
        ...
        if ($ageYears === false) {
            $errors[] = 'Vek musí byť celé číslo od 20 do 80 rokov (validačný rozsah modelu).';
        }
        ...
        if ($egfr === null || $egfr > 200) {
            $errors[] = 'eGFR musí byť kladné číslo v rozsahu 0–200 ml/min/1,73 m².';
        }
        ...
        // Fajčenie
        $smoking = in_array($form['smoking'], ['never', 'former', 'current'], true) ? $form['smoking'] : '';
        if ($smoking === '') {
        ...
        if ($diabetes) {
            ...
            if ($hba1cRaw === null || $hba1cRaw < 4.0 || $hba1cRaw > 20.0) {
                $errors[] = 'HbA1c musí byť číslo v rozsahu 4–20 % (len pri DM).';
            } else {
                $hba1c = $hba1cRaw;
            }
        ...
        if (empty($errors)) {
            ...
            if ($uacrMgG < 0.5 || !is_finite($uacrMgG)) {
                $errors[] = 'Hodnota UACR je príliš nízka alebo neplatná po konverzii.';
            } else {
                ...
                if ($action === 'save') {
                    if (!isLoggedIn()) {
                        ...
                        try {
                            ...
                            error_log('calculator_ckdpc save error: ' . $e->getMessage());
                        }
                    }
                }
            }
...
if (isLoggedIn()) {
    try {
        $savedResults = calculatorFetchSavedResults($pdo, (int) $_SESSION['user_id'], 'ckdpc_grams_2022', 25);
    } catch (\PDOException $e) {
...
/* ─────────────────────────────────────────────────────
   ...
   ───────────────────────────────────────────────────── */
function boolLabel(bool $v): string { return $v ? 'Áno' : 'Nie'; }
function smokingLabel(string $v): string {
    return match ($v) { 'current' => 'Fajčí', 'former' => 'Ex-fajčiar', default => 'Nefajčí' };
}
function sexLabel(string $v): string { return $v === 'male' ? 'Muž' : 'Žena'; }
?>
<!DOCTYPE html>
<html lang="sk">
<head>
...
<body>
    ...
    $headerIntro = 'CKD Prognosis Consortium — 3-ročné riziko progresie CKD (Grams 2022)';
    $showLogo    = false;
    include 'header.php';
    ?>
    ...
    <nav class="main-nav" aria-label="Hlavná navigácia">
        <div class="container">
            <ul>
                ...
                <li><a href="calculator_kfre.php">KFRE</a></li>
                <?php if (isLoggedIn()): ?>
                    <?php if (isAdmin()): ?>
...
&eta; = &beta;<sub>0</sub> + &beta;<sub>1</sub>&middot;Vek + &beta;<sub>2</sub>&middot;muž + &beta;<sub>3</sub>&middot;ln(eGFR) + &beta;<sub>4</sub>&middot;ln(uACR) + &beta;<sub>5</sub>&middot;SBP
    ...
    + [&beta;<sub>12</sub>&middot;HbA1c + &beta;<sub>13</sub>&middot;inzulín + &beta;<sub>14</sub>&middot;PAD] &nbsp;(len pri DM)</code>
                ...
                <form method="POST" action="calculator_ckdpc.php" id="ckdpc-form">
                        ...
                        <div class="form-grid">
                            <div class="form-group">
                                ...
                                <input type="text" id="patient_first_name" name="patient_first_name"
                                       class="form-control"
                                       value="<?= htmlspecialchars($form['patient_first_name']) ?>">
                            </div>
                            <div class="form-group">
                                <label for="patient_last_name">Priezvisko</label>
                                <input type="text" id="patient_last_name" name="patient_last_name"
                                       class="form-control"
                                       value="<?= htmlspecialchars($form['patient_last_name']) ?>">
                            </div>
                            <div class="form-group">
                                <label for="patient_birth_date">Dátum narodenia</label>
                                <input type="date" id="patient_birth_date" name="patient_birth_date"
                                       class="form-control"
                                       value="<?= htmlspecialchars($form['patient_birth_date']) ?>">
                            </div>
                            <div class="form-group">
                                <label for="patient_birth_number">Rodné číslo</label>
                                <input type="text" id="patient_birth_number" name="patient_birth_number"
                                       class="form-control" placeholder="000000/0000"
                                       value="<?= htmlspecialchars($form['patient_birth_number']) ?>">
                            </div>
                            <div class="form-group">
                        ...
                        <div class="form-grid">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" id="egfr" name="egfr" required class="form-control"
                                       value="<?= htmlspecialchars($form['egfr']) ?>">
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <div style="display:flex;gap:8px;">
                                    <input type="text" id="uacr_value" name="uacr_value" required
                                           ...
                                           value="<?= htmlspecialchars($form['uacr_value']) ?>">
                        ...
                        <div class="form-grid">
                            ...
                            <div class="form-group">
                                ...
                                <select id="antihtn" name="antihtn" class="form-control">
                                    <option value="0" <?= $form['antihtn'] === '0' ? 'selected' : '' ?>>Nie</option>
                                    <option value="1" <?= $form['antihtn'] === '1' ? 'selected' : '' ?>>Áno</option>
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select id="hf" name="hf" class="form-control">
                                    <option value="0" <?= $form['hf'] === '0' ? 'selected' : '' ?>>Nie</option>
                                    <option value="1" <?= $form['hf'] === '1' ? 'selected' : '' ?>>Áno</option>
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select id="chd" name="chd" class="form-control">
                                    <option value="0" <?= $form['chd'] === '0' ? 'selected' : '' ?>>Nie</option>
                                    <option value="1" <?= $form['chd'] === '1' ? 'selected' : '' ?>>Áno</option>
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select id="afib" name="afib" class="form-control">
                                    <option value="0" <?= $form['afib'] === '0' ? 'selected' : '' ?>>Nie</option>
                                    <option value="1" <?= $form['afib'] === '1' ? 'selected' : '' ?>>Áno</option>
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select id="smoking" name="smoking" class="form-control">
                                    <option value="never"   <?= $form['smoking'] === 'never'   ? 'selected' : '' ?>>Nefajčí</option>
                                    <option value="former"  <?= $form['smoking'] === 'former'  ? 'selected' : '' ?>>Ex-fajčiar</option>
                                    <option value="current" <?= $form['smoking'] === 'current' ? 'selected' : '' ?>>Fajčí</option>
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select id="diabetes" name="diabetes" class="form-control"
                                    ...
                                    <option value="0" <?= $form['diabetes'] === '0' ? 'selected' : '' ?>>Nie</option>
                                    <option value="1" <?= $form['diabetes'] === '1' ? 'selected' : '' ?>>Áno</option>
                                </select>
                            </div>
                    ...
                    <!-- DM-špecifické polia -->
                    <div class="form-section" id="dm-fields" style="<?= $form['diabetes'] === '1' ? '' : 'display:none' ?>">
                        ...
                        <div class="form-grid">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" id="hba1c" name="hba1c" class="form-control"
                                       value="<?= htmlspecialchars($form['hba1c']) ?>"
                            ...
                            <div class="form-group">
                                ...
                                <select id="insulin" name="insulin" class="form-control">
                                    <option value="0" <?= $form['insulin'] === '0' ? 'selected' : '' ?>>Nie</option>
                                    <option value="1" <?= $form['insulin'] === '1' ? 'selected' : '' ?>>Áno</option>
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select id="oral_dm" name="oral_dm" class="form-control">
                                    <option value="0" <?= $form['oral_dm'] === '0' ? 'selected' : '' ?>>Nie</option>
                                    <option value="1" <?= $form['oral_dm'] === '1' ? 'selected' : '' ?>>Áno</option>
                                </select>
                            </div>
                ...
                <?php if ($calculated !== null): ?>
                    <div class="form-section calculator-result-block ckdpc-result">
                        ...
                        <p class="helper-text">Model: <strong><?= htmlspecialchars($calculated['model_name']) ?></strong></p>

                        <div class="ckdpc-risk-display">
                            <div class="risk-item risk-3yr">
                                <div class="risk-label">3-ročné riziko progresie</div>
                                <div class="risk-value">
                                    <?= htmlspecialchars(number_format((float) $calculated['risk_3yr'], 1, ',', ' ')) ?> %
                                </div>
                        ...
                        <details class="ckdpc-inputs-summary">
                            ...
                            <table class="admin-table" style="margin-top:8px;">
                                <tr><th>Vek</th><td><?= (int) $calculated['age_years'] ?> rokov</td></tr>
                                <tr><th>Pohlavie</th><td><?= htmlspecialchars(sexLabel($calculated['sex'])) ?></td></tr>
                                <tr><th>eGFR</th><td><?= htmlspecialchars((string) $calculated['egfr']) ?> ml/min/1,73 m²</td></tr>
                                <tr><th>UACR</th><td>
                                    <?= htmlspecialchars(number_format((float) $calculated['uacr_input'], 2, ',', ' ')) ?>
                                    <?= $calculated['uacr_unit'] === 'mg_mmol' ? 'mg/mmol' : 'mg/g' ?>
                                    <?php if ($calculated['uacr_unit'] === 'mg_mmol'): ?>
                                        (= <?= htmlspecialchars(number_format((float) $calculated['uacr_mg_g'], 1, ',', ' ')) ?> mg/g)
                                    <?php endif; ?>
                                </td></tr>
                                <tr><th>DM</th><td><?= boolLabel($calculated['diabetes']) ?></td></tr>
                                <tr><th>Systolický TK</th><td><?= (int) $calculated['sbp'] ?> mmHg</td></tr>
                                <tr><th>Antihypertenzíva</th><td><?= boolLabel($calculated['antihtn']) ?></td></tr>
                                <tr><th>Srdcové zlyhanie</th><td><?= boolLabel($calculated['hf']) ?></td></tr>
                                <tr><th>ICHS</th><td><?= boolLabel($calculated['chd']) ?></td></tr>
                                <tr><th>Fibrilácia predsiení</th><td><?= boolLabel($calculated['afib']) ?></td></tr>
                                <tr><th>BMI</th><td><?= htmlspecialchars((string) $calculated['bmi']) ?> kg/m²</td></tr>
                                <tr><th>Fajčenie</th><td><?= htmlspecialchars(smokingLabel($calculated['smoking'])) ?></td></tr>
                                <?php if ($calculated['diabetes']): ?>
                                    <tr><th>HbA1c</th><td><?= htmlspecialchars(number_format((float) $calculated['hba1c'], 1, ',', ' ')) ?> %</td></tr>
                                    <tr><th>Inzulín</th><td><?= boolLabel($calculated['insulin']) ?></td></tr>
                                    <tr><th>Perorálna antidiabetická liečba</th><td><?= boolLabel($calculated['oral_dm']) ?></td></tr>
                                <?php endif; ?>
                            </table>
                        </details>
                                ...
                                <table class="admin-table">
                                    ...
                                    <tbody>
                                        <?php foreach ($savedResults as $row): ?>
                                            <tr>
                                                <td><?= htmlspecialchars(substr((string) ($row['created_at'] ?? ''), 0, 16)) ?></td>
                                                <td><?= htmlspecialchars(calculatorBuildPatientDisplay($row)) ?></td>
                                                <td><?= htmlspecialchars((string) ($row['result_payload']['model_name'] ?? '–')) ?></td>
                                                <td>
                                                    <strong>
                                                        <?= htmlspecialchars(
                                                            number_format((float) ($row['result_payload']['risk_3yr'] ?? 0), 1, ',', ' ')
                                                        ) ?> %
                                                    </strong>
                                                </td>
                                                <td>
